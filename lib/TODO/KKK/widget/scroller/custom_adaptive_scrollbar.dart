...
/// in a [CustomAdaptiveScrollbar] and specify the [ScrollController] of your [ScrollView].
///
...
/// 自定义 Scrollbar，
/// 参考[https://github.com/rulila52/adaptive-scrollbar/tree/main]
class CustomAdaptiveScrollbar extends StatefulWidget {
  ...
  /// Position of [CustomAdaptiveScrollbar] on the screen.
  ...
  CustomAdaptiveScrollbar({
    ...
    if (underDecoration == null) {
      this.underDecoration = BoxDecoration(shape: BoxShape.rectangle, color: underColor);
    } else {
      this.underDecoration = underDecoration;
    }

    if (sliderDecoration == null) {
      this.sliderDecoration = BoxDecoration(shape: BoxShape.rectangle, color: sliderDefaultColor);
    } else {
    ...
    if (sliderActiveDecoration == null) {
      this.sliderActiveDecoration =
          BoxDecoration(shape: BoxShape.rectangle, color: this.sliderActiveColor);
    } else {
  ...
  @override
  _CustomAdaptiveScrollbarState createState() => _CustomAdaptiveScrollbarState();
}

class _CustomAdaptiveScrollbarState extends State<CustomAdaptiveScrollbar> {
                          ...
                          child: GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: ScrollSlider(
                                  ...
                                  scrollToClickFirstDelay: widget.scrollToClickFirstDelay,
                                  scrollToClickOtherDelay: widget.scrollToClickOtherDelay,
                                  ...
                                  sliderActiveDecoration: widget.sliderActiveDecoration,
                                ),
                              )))));
        }),
      ]),
...
class _ScrollSliderState extends State<ScrollSlider> {
  ...
  double viewMaxScrollDuringBuild(double maxHeight) => widget.controller.position.maxScrollExtent;
  ...
  void scrollToClick(double position, ToClickDirection direction) async {
    ...
    timer = Timer(
        Duration(
            milliseconds:
                isFirst ? widget.scrollToClickFirstDelay : widget.scrollToClickOtherDelay), () {
      isFirst = false;
      if (sliderOffset + finalSliderHeight < position && direction == ToClickDirection.down) {
  ...
  void onDragUpdate(DragUpdateDetails details) {
    setState(() {
      ...
      double viewDelta = getScrollViewDelta(details.delta.dy, sliderMaxScroll, viewMaxScroll);
  ...
  onScrollUpdate() {
    ...
    super.setState(() {
      setState(() {
        sliderOffset = widget.controller.position.pixels / viewMaxScroll * sliderMaxScroll;
    ...
    return LayoutBuilder(builder: (context, constraints) {
      finalSliderHeight = widget.sliderHeight ??
          constraints.maxHeight *
                  constraints.maxHeight /
                  (constraints.maxHeight + viewMaxScrollDuringBuild(constraints.maxHeight)) -
