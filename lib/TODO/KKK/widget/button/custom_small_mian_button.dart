...
class CustomSmallMainButton extends StatelessWidget {
  ...
  // final double contentPaddingTopAndBottom  ;
  final EdgeInsetsGeometry? padding;//todo: 改为padding 更加灵活
  ...
  const CustomSmallMainButton({
    ...
    this.textTheme,
    // this.contentPaddingTopAndBottom = 0,
    this.padding,
  }) : super(key: key);
      ...
      builder: (_, con) {
        TextStyle style = textStyle ??
            defaultTextTheme!.bodyMedium!
                .copyWith(color: _getTextColor(defaultThemeData),fontWeight: fontWeight);
        if (fontSize != null) {
        ...
        return CustomButton(
          // padding: EdgeInsets.only(top: contentPaddingTopAndBottom,bottom: contentPaddingTopAndBottom),
  ...
  Color _getBackgroundColor(ButtonThemeData theme) {
    return backgroundColor ??
        ...
        ThemeService().theme.colorScheme.onPrimary;
  }

  Color _getTextColor(ButtonThemeData theme) {
    if(isEnable) {
      return textColor ?? theme.whiteColor;
    }else{
      return textColor != null ? textColor!.withOpacity(0.7) : theme.whiteColor.withOpacity(0.7);
    }
  }
}
