...
import 'package:flutter_bet/generated/intl/messages_en.dart';

const double _kTabHeight = 46.0;

///tab数量较少时为平均分配
class CoveredTabBar extends StatefulWidget implements PreferredSizeWidget {
  CoveredTabBar({
    Key? key,
    required this.tabs,
    this.controller,
...
class _ButtonsTabBarState extends State<CoveredTabBar> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(CoveredTabBar oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    var margin  = widget.sideMargin!.left + widget.sideMargin!.right;
    double wid = MediaQuery.of(context).size.width - margin ;
    // log("margin ${margin}");
    // log("wid ${wid}");
    // log("wid-height ${widget.preferredSize.height}");
    // log("wid-_centerPadding ${_centerPadding}");
    double buttonWidth =wid / (widget.tabs.length ) ;
    // log("buttonWidth ${buttonWidth}");
    return Opacity(
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              ...
              children: List.generate(
                widget.tabs.length,
                (int index) => SizedBox(
                    width: buttonWidth, child: _buildButton(index, widget.tabs[index] as Tab)),
              ),
            ),
          ),
        ),
