...
class DepositWithdrawalNotifUtils {
  ...
  static Widget _subView(String amount, String coinId, String subtitle) {
    return LangUtil().isViVn() ? _vivnLanguage(amount, coinId, subtitle) :
    LangUtil().isTh()
  ...
  static Widget _vivnLanguage(String amount, String coinId, String subtitle) {
    return RichText(
        text: TextSpan(children: [
          WidgetSpan(
              child: Row(
                children: [
                  CustomText(
                    "${S.current.vault_amount}${amount}", //amount
                    textColorStyle: CustomTextColorStyle.normal,
                  ),
                  const SizedBox(
                    width: 4,
