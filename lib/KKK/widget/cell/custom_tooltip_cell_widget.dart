...
///联盟计划= 列表基本表格的单元
///基本功能是是超过指定大小，显示... 并触碰弹出Tooltip
class CustomTooltipCellWidget extends StatelessWidget {
  const CustomTooltipCellWidget({
    super.key,
    this.content,
    this.maxLength = 6,
    this.padding,
  ...
  Widget build(BuildContext context) {
    //...处理
    var contentTarget = TextUtil.clipping(content, maxLength);
    return CustomTooltipWidget(
        isShow: contentTarget.length > maxLength,
        padding: EdgeInsets.zero,
        verticalOffset: 10,
        decoration: const BoxDecoration(color: Colors.transparent),
        richMessage: WidgetSpan(
          // alignment: PlaceholderAlignment.bottom,
          child: _buildTipContentView(content),
        ),
        child: Container(
  ...
  _buildTipContentView(String? content) {
    return UnconstrainedBox(
      child: BubbleView(
        ...
        child: CustomText(
          content ?? '',
          style: ThemeService().textTheme.labelMedium?.copyWith(
