...
class ButtonScrollTabBar extends StatefulWidget implements PreferredSizeWidget {
  ButtonScrollTabBar({
    Key? key,
    required this.tabs,
    this.controller,
...
class _ButtonsTabBarState extends State<ButtonScrollTabBar> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(ButtonScrollTabBar oldWidget) {
  ...
  TextStyle _getStyle(int index, double animationValue) {
    ...
    if (widget.isSport == true && widget.isLogin == false && ((index == 1) || (index == 2))) {
  ...
  Widget _buildButton(
    ...
    final TextStyle? textStyle = TextStyle.lerp(
        ...
        animationValue);
    final Color? borderColor =
        Color.lerp(widget.unselectedBorderColor, widget.borderColor, animationValue);
    ...
    final BoxDecoration? boxDecoration = BoxDecoration.lerp(
        BoxDecoration(
          ...
          borderRadius:
              widget.unselectedDecoration?.borderRadius ?? BorderRadius.circular(widget.radius),
        ),
        BoxDecoration(
          ...
          child: Container(
            ...
            child: Row(
              children: <Widget>[
                tab.icon != null
                    ? IconTheme.merge(
                        data: IconThemeData(size: 24.0, color: foregroundColor), child: tab.icon!)
                    : Container(),
                SizedBox(
                ...
                // widget.isSport == true ? cannotTextStyle! : textStyle!
                tab.text != null
                    ? richText(tab.text!, _getStyle(index, animationValue), tab)
                    : (tab.child ?? Container()),
                Visibility(
                    visible: _isRaceLeaderboardKey(tab.key),
                    child: SizedBox(
                      width: 4,
                    )),
  ...
  Widget richText(String text, TextStyle textStyle, Tab tab) {
    if (text.contains('/')) {
  ...
  bool _isRaceLeaderboardKey(Key? key) {
    if (key is ValueKey<String>) {
      return key.value == 'raceLeaderboard';
    }
    return false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        ...
        decoration: BoxDecoration(
          color: ThemeService().colorScheme.surface, // 设置背景色
          borderRadius: BorderRadius.horizontal(
              left: Radius.circular((_currentIndex == 0 || _currentIndex == 1) ? 27 : 0),
              right: Radius.circular((_currentIndex == (widget.tabs.length - 1) ||
                      _currentIndex == (widget.tabs.length - 2))
                  ? 27
                  : 0)),
        ),
  ...
  _scrollTo(int index) {
    ...
    if (offset < 0) {
      ...
      renderBox = (_textLTR ? _tabKeys.first : _tabKeys.last).currentContext?.findRenderObject()
          as RenderBox;
      ...
      // get the last button
      renderBox = (_textLTR ? _tabKeys.last : _tabKeys.first).currentContext?.findRenderObject()
          as RenderBox;
      // get its position
      position = renderBox.localToGlobal(tabsContainerOffset).dx;
      // and size
    ...
    _scrollController.animateTo(offset + _scrollController.offset,
        duration: Duration(milliseconds: widget.duration), curve: Curves.easeInOut);
  }
}
